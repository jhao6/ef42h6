...
def get(seed=0, fixed_order=False, pc_valid=0.15, base_dir=None, label_noise=False, noise_rate=0.2):
    data={}
    taskcla=[]
    size=  [3,32,32]
            ...
            if idx==0:
                ...
                for s in ['train','test']:
                    ...
                    for image,target in loader:
                        ...
                        data[n][s]['y'].append(target.numpy()[0])
                if label_noise:
                    noise_idxs = np.random.randint(0, len(data[n][s]['y']), size=int(noise_rate*len(data[n][s]['y'])))
                    for i in noise_idxs:
                        data[n][s]['y'][i] = np.random.randint(0, max(data[n][s]['y'])+1)
                    # np.array(data[n][s]['y'])[noise_idxs] = np.random.randint(0, max(data[n][s]['y'])+1, size=int(0.2*len(data[n][s]['y'])))
